...
- LeoStudio integration: loading from `/infinite-canvas/` should keep all hash routes and static assets working, automatically select the local OpenAI-compatible channel, discover the bundled image/video models, and create/poll a video task through the local proxy.
