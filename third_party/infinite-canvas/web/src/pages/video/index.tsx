...
export default function VideoPage() {
    ...
    const generate = async () => {
        ...
        try {
            const clientRequestId = `video-page-${nanoid()}`;
            const task = await createVideoGenerationTask(snapshot.config, snapshot.text, snapshot.references, snapshot.videoReferences, snapshot.audioReferences, { clientRequestId });
    ...
    const pollGenerationLog = async (log: GenerationLog, configOverride?: AiConfig, agentTaskId?: string) => {
        ...
        try {
            // Do not mark a persisted task as failed merely because it has
            // spent five minutes in Leonardo's queue. Continue polling the
            // same task so page reload/resume never creates a duplicate job.
            for (;;) {
                ...
                if (state.status === "completed") {
                    ...
                    const nextVideo: GeneratedVideo = {
                        ...
                        creditCost: state.result.creditCost,
                    };
        ...
        <div className="overflow-hidden rounded-lg border border-stone-200 bg-background dark:border-stone-800">
            ...
            <div className="flex flex-wrap items-center justify-between gap-x-3 gap-y-2 border-t border-stone-200 px-3 py-2.5 dark:border-stone-800">
                <div className="flex min-w-0 flex-wrap gap-x-2 gap-y-1 text-xs text-stone-500 dark:text-stone-400">
                    ...
                    {typeof video.creditCost === "number" && <span>消耗 {video.creditCost} 积分</span>}
                </div>
