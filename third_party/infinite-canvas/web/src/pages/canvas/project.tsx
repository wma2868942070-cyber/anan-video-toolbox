...
function InfiniteCanvasPage() {
    ...
    const maskEditImageNode = useCallback(
        async (node: CanvasNodeData, payload: CanvasImageMaskEditPayload) => {
            ...
            const childId = nanoid();
            const clientRequestId = `canvas-image-${childId}-${nanoid()}`;
            ...
            setNodes((prev) => [
                ...prev,
                {
                    ...
                    width: node.width,
                    height: node.height,
                    metadata: { prompt, status: NODE_STATUS_LOADING, clientRequestId, ...generationMetadata },
                },
            ]);
            setConnections((prev) => [...prev, { id: nanoid(), fromNodeId: node.id, toNodeId: childId }]);
            setSelectedNodeIds(new Set([childId]));
            ...
            try {
                const image = await requestEdit(generationConfig, prompt, [source], { id: `${node.id}-mask`, name: "mask.png", type: "image/png", dataUrl: payload.maskDataUrl }, { signal: controller.signal, clientRequestId }).then((items) => items[0]);
        ...
        async (node: CanvasNodeData, params: CanvasImageAngleParams) => {
            ...
            try {
                const image = await requestEdit(
                    ...
                    [{ id: node.id, name: `${node.title || node.id}.png`, type: node.metadata.mimeType || "image/png", dataUrl: node.metadata.content, storageKey: node.metadata.storageKey }],
                    undefined,
                    { signal: controller.signal, clientRequestId },
                ).then((items) => items[0]);
                const uploaded = await uploadImage(image.dataUrl);
    ...
    const handleGenerateNode = useCallback(
        async (nodeId: string, mode: CanvasNodeGenerationMode, prompt: string) => {
            ...
            if (sourceNode && builtinPanel?.writeBackToSelf && builtinPanel.mode === "image") {
                ...
                const clientRequestId = `canvas-image-${nodeId}-${nanoid()}`;
                setNodes((prev) => prev.map((node) => (node.id === nodeId ? { ...node, metadata: { ...node.metadata, prompt: scene, clientRequestId, status: NODE_STATUS_LOADING, errorDetails: undefined } } : node)));
                try {
                    ...
                    const image = refs.length
                        ? await requestEdit({ ...generationConfig, count: "1" }, fullPrompt, refs, undefined, { signal: controller.signal, clientRequestId }).then((items) => items[0])
                        : await requestGeneration({ ...generationConfig, count: "1" }, fullPrompt, { signal: controller.signal, clientRequestId }).then((items) => items[0]);
                    const uploaded = await uploadImage(image.dataUrl);
                    setNodes((prev) =>
            ...
            try {
                if (mode === "image") {
                    ...
                    const rootNode: CanvasNodeData = {
                        ...
                        metadata: {
                            ...
                            images: imageIds.map((id) => ({ id, clientRequestId: `canvas-image-${rootId}-${id}`, status: NODE_STATUS_LOADING, content: "", storageKey: "", naturalWidth: 0, naturalHeight: 0, bytes: 0, mimeType: "" })),
                            ...generationMetadata,
                        },
                    };
                    ...
                    await Promise.all(
                        imageIds.map(async (imageId) => {
                            try {
                                const clientRequestId = `canvas-image-${rootId}-${imageId}`;
                                const image = referenceImages.length
                                    ? await requestEdit({ ...generationConfig, count: "1" }, effectivePrompt, referenceImages, undefined, { signal: controller.signal, clientRequestId }).then((items) => items[0])
                                    : await requestGeneration({ ...generationConfig, count: "1" }, effectivePrompt, { signal: controller.signal, clientRequestId }).then((items) => items[0]);
                                const uploaded = await uploadImage(image.dataUrl);
                ...
                if (mode === "video") {
                    ...
                    const clientRequestId = `canvas-video-${videoId}-${nanoid()}`;
                    ...
                    try {
                        const video = await storeGeneratedVideo(
                            await requestVideoGeneration(generationConfig, effectivePrompt, generationContext.referenceImages, generationContext.referenceVideos, generationContext.referenceAudios, { signal: controller.signal, clientRequestId }),
                        );
    ...
    const handleRetryNode = useCallback(
        async (node: CanvasNodeData, imageId?: string) => {
            ...
            const existingRetryImage = imageId ? node.metadata?.images?.find((item) => item.id === imageId) : undefined;
            const retryClientRequestId =
                node.type === CanvasNodeType.Video
                    ? node.metadata?.content
                        ? `canvas-video-${node.id}-${nanoid()}`
                        : node.metadata?.clientRequestId || `canvas-video-${node.id}-${nanoid()}`
                    : node.metadata?.content || existingRetryImage?.content
                      ? `canvas-image-${node.id}-${nanoid()}`
                      : existingRetryImage?.clientRequestId || node.metadata?.clientRequestId || `canvas-image-${node.id}-${nanoid()}`;
                    ...
                    item.id === node.id
                        ? {
                              ...
                              metadata: {
                                  ...
                                  status: NODE_STATUS_LOADING,
                                  errorDetails: undefined,
                                  images: item.metadata?.images?.map((image) => (image.id === imageId ? { ...image, clientRequestId: retryClientRequestId, status: NODE_STATUS_LOADING, errorDetails: undefined } : image)),
                              },
                          }
                        : item,
                ),
            ...
            try {
                ...
                if (node.type === CanvasNodeType.Video) {
                    const video = await storeGeneratedVideo(
                        await requestVideoGeneration(generationConfig, prompt, retryImages, context?.referenceVideos || [], context?.referenceAudios || [], { signal: controller.signal, clientRequestId: retryClientRequestId }),
                    );
                ...
                const image = useReferenceImages
                    ? await requestEdit(generationConfig, prompt, retryImages, undefined, { signal: controller.signal, clientRequestId: retryClientRequestId }).then((items) => items[0])
                    : await requestGeneration(generationConfig, prompt, { signal: controller.signal, clientRequestId: retryClientRequestId }).then((items) => items[0]);
