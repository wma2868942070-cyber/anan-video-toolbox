...
export function ModelSelectModal({ open, channel, selectedNames, onConfirm, onClose }: { open: boolean; channel: ModelChannel | null; selectedNames: string[]; onConfirm: (names: string[]) => void; onClose: () => void }) {
    ...
    const displayNames = useMemo(() => new Map((channel?.models || []).map((model) => [model.name, model.displayName || model.name])), [channel?.models]);
    ...
    const visibleList = useMemo(() => {
        ...
        return keyword ? currentList.filter((name) => name.toLowerCase().includes(keyword) || (displayNames.get(name) || "").toLowerCase().includes(keyword)) : currentList;
    }, [currentList, displayNames, search]);
