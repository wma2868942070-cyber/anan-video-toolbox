...
/**
 * A stable idempotency key lets the local Leonardo/Adobe gateway reuse a
 * completed generation when the canvas page is reloaded or a request is
 * retried after the browser was closed.
 */
...
export async function requestEdit(config: AiConfig, prompt: string, references: ReferenceImage[], mask?: ReferenceImage, options?: RequestOptions) {
    ...
    if (options?.clientRequestId) formData.set("client_request_id", options.clientRequestId);
