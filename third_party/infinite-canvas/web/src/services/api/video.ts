...
type VideoProvider = { credit_cost?: number };
type VideoResponse = {
    id: string;
    status?: string;
...
export type RequestOptions = { signal?: AbortSignal; clientRequestId?: string };
const apiText = (key: string, options?: Record<string, unknown>) => i18n.t(`apiErrors.${key}`, options);

export type VideoGenerationResult = { blob?: Blob; url?: string; mimeType?: string; creditCost?: number };
export type VideoGenerationTask = { id: string; provider: "openai" | "adobe" | "seedance" | "plugin"; model: string; clientRequestId?: string };
...
export async function requestVideoGeneration(config: AiConfig, prompt: string, references: ReferenceImage[] = [], videoReferences: ReferenceVideo[] = [], audioReferences: ReferenceAudio[] = [], options?: RequestOptions): Promise<VideoGenerationResult> {
    ...
    const delayMs = task.provider === "seedance" || task.provider === "adobe" ? 5000 : 2500;
    // Video providers can remain queued for substantially longer than five
    // minutes. Keep following the existing task until the provider returns a
    // terminal state or the user explicitly cancels the canvas request. A
    // fixed client-side timeout would abandon a still-running Leonardo job and
    // make the retry button submit a duplicate generation.
    for (;;) {
...
export async function createVideoGenerationTask(config: AiConfig, prompt: string, references: ReferenceImage[] = [], videoReferences: ReferenceVideo[] = [], audioReferences: ReferenceAudio[] = [], options?: RequestOptions): Promise<VideoGenerationTask> {
    ...
    if (isAdobeVideoConfig(requestConfig, selectedModel)) {
        return createAdobeVideoTask(requestConfig, selectedModel, prompt, references, videoReferences, audioReferences, options);
    }
...
function isAdobeVideoConfig(config: AiConfig, model: string) {
    const baseUrl = config.baseUrl.trim().replace(/\/+$/, "").toLowerCase();
    return baseUrl.endsWith("/adobe") || modelOptionName(model).toLowerCase().startsWith("firefly-");
}

async function createAdobeVideoTask(config: AiConfig, model: string, prompt: string, references: ReferenceImage[], videoReferences: ReferenceVideo[], audioReferences: ReferenceAudio[], options?: RequestOptions): Promise<VideoGenerationTask> {
    ...
    try {
        ...
        return { id: created.id, provider: "adobe", model, clientRequestId: options?.clientRequestId };
    } catch (error) {
...
async function createPluginVideoTask(config: AiConfig, model: string, script: string, prompt: string, references: ReferenceImage[], options?: RequestOptions): Promise<VideoGenerationTask> {
    ...
    return { id, provider: "plugin", model, clientRequestId: options?.clientRequestId };
}
...
async function createOpenAIVideoTask(config: AiConfig, model: string, prompt: string, references: ReferenceImage[], options?: RequestOptions): Promise<VideoGenerationTask> {
    ...
    const generateAudio = String(boolConfig(config.videoGenerateAudio, true));
    body.append("generate_audio", generateAudio);
    body.append("audio", generateAudio);
    body.append("preset", "normal");
    if (options?.clientRequestId) body.append("client_request_id", options.clientRequestId);
    const files = await Promise.all(references.slice(0, 7).map(async (image) => dataUrlToFile({ ...image, dataUrl: await imageToDataUrl(image) })));
    files.forEach((file) => body.append("input_reference[]", file));
    try {
        const created = unwrapVideoResponse((await axios.post<ApiVideoResponse>(aiApiUrl(config, "/videos"), body, { headers: aiHeaders(config), signal: options?.signal })).data);
        if (!created.id) throw new Error(apiText("noVideoTaskId"));
        return { id: created.id, provider: "openai", model, clientRequestId: options?.clientRequestId };
    } catch (error) {
        throw new Error(readAxiosError(error, apiText("videoTaskCreateFailed")));
    }
}
...
async function pollOpenAIVideoTask(config: AiConfig, task: VideoGenerationTask, options?: RequestOptions): Promise<VideoGenerationTaskState> {
    try {
        ...
        const creditCost = video.result?.provider?.credit_cost ?? video.provider?.credit_cost;
        const url = videoResultUrl(video);
        if (url) return { status: "completed", result: await videoResultFromUrl(url, options, creditCost) };
        if (video.status === "completed") {
            const content = await axios.get<Blob>(aiApiUrl(config, `/videos/${task.id}/content`), { headers: aiHeaders(config), responseType: "blob", signal: options?.signal });
            await assertVideoBlob(content.data);
            return { status: "completed", result: { blob: content.data, creditCost } };
        }
...
async function createSeedanceTask(config: AiConfig, model: string, prompt: string, references: ReferenceImage[], videoReferences: ReferenceVideo[], audioReferences: ReferenceAudio[], options?: RequestOptions): Promise<VideoGenerationTask> {
    ...
    try {
        ...
        return { id: created.id, provider: "seedance", model, clientRequestId: options?.clientRequestId };
    } catch (error) {
...
async function videoResultFromUrl(url: string, options?: RequestOptions, creditCost?: number): Promise<VideoGenerationResult> {
    try {
        const response = await axios.get<Blob>(url, { responseType: "blob", signal: options?.signal });
        await assertVideoBlob(response.data);
        return { blob: response.data, creditCost };
    } catch (error) {
        if (axios.isCancel(error) || options?.signal?.aborted) throw error;
        return { url, mimeType: "video/mp4", creditCost };
    }
}
